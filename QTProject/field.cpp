        ...
        for( int j = 0; j < 8; j++ )
        {
            if( ( i%2 != 0 && j%2 != 0 ) || ( i%2 == 0 && j%2 == 0 ) )
            {
                ...
                if( !GameCells[i][j/2]->isEmpty() )
                {
                    ...
                    {
                        fig = scene->addEllipse(x, y, wigth, height, fieldpen, redbrush );
                        fig->setFlags(QGraphicsItem::ItemIsMovable);
                    }

                    if( GameCells[i][j/2]->getFigure().getColor() == COLOR_BLUE )
                    {
                        fig = scene->addEllipse(x, y, wigth, height, fieldpen, bluebrush );
                        fig->setFlags(QGraphicsItem::ItemIsMovable);
                    }
                }
            }
...
bool Field::canBeat(const Cell *arg)
{
...
bool Field::qcanBeat(const Cell *arg)
{
